...
describe('CallStateMachine', () => {
  ...
  describe('room и participantName', () => {
    it('возвращают undefined в IDLE', () => {
      expect(machine.room).toBeUndefined();
      expect(machine.participantName).toBeUndefined();
    });

    it('возвращают undefined в CONNECTING до enter-room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
    ...
    it('возвращают значения в PURGATORY после enter-room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...purgatoryPayload });

      expect(machine.state).toBe(EState.PURGATORY);
      expect(machine.room).toBe(purgatoryPayload.room);
      expect(machine.participantName).toBe(purgatoryPayload.participantName);
    });

    it('возвращают значения в P2P_ROOM после enter-room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...p2pRoomPayload });

      expect(machine.state).toBe(EState.P2P_ROOM);
      expect(machine.room).toBe(p2pRoomPayload.room);
      expect(machine.participantName).toBe(p2pRoomPayload.participantName);
    });

    it('возвращают значения в DIRECT_P2P_ROOM после enter-room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...directP2pRoomPayload });

      expect(machine.state).toBe(EState.DIRECT_P2P_ROOM);
      expect(machine.room).toBe(directP2pRoomPayload.room);
      expect(machine.participantName).toBe(directP2pRoomPayload.participantName);
    });

    it('возвращают значения в IN_ROOM после enter-room и token', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });
      machine.send({ type: 'CALL.TOKEN_ISSUED', token: token1Payload.jwt });

      expect(machine.state).toBe(EState.IN_ROOM);
      expect(machine.room).toBe(room1Payload.room);
      expect(machine.participantName).toBe(room1Payload.participantName);
    });

    it('обновляются при смене room через enter-room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });
      ...
      expect(machine.room).toBe(room2Payload.room);
      expect(machine.participantName).toBe(room2Payload.participantName);
    });
  });
