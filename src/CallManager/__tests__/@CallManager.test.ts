...
import { EState } from '../CallStateMachine';
...
describe('CallManager', () => {
  ...
  describe('enter-room event', () => {
    it('эмитит enter-room при переходе в PURGATORY с room и participantName', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();

      cm.on(EEvent.ENTER_ROOM, enterRoomSpy);
      cm.events.trigger('start-call', { number: '100', answer: false });
      ...
      expect(cm.stateMachine.state).toBe(EState.PURGATORY);
      expect(enterRoomSpy).toHaveBeenCalledWith({
        room: PURGATORY_CONFERENCE_NUMBER,
        participantName: 'User',
        isInPurgatory: true,
        isP2PRoom: false,
        isDirectP2PRoom: false,
      });
    });

    it('эмитит enter-room при переходе в IN_ROOM с bearerToken', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();
      ...
      expect(cm.stateMachine.state).toBe(EState.IN_ROOM);
      expect(enterRoomSpy).toHaveBeenCalledWith({
        room: 'room-1',
        participantName: 'User',
        isInPurgatory: false,
    ...
    it('эмитит enter-room при переходе в P2P_ROOM', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();
      ...
      expect(cm.stateMachine.state).toBe(EState.P2P_ROOM);
      expect(enterRoomSpy).toHaveBeenCalledWith({
        room: 'p2pCallerToCallee',
        participantName: 'User',
        isInPurgatory: false,
    ...
    it('эмитит enter-room при переходе в DIRECT_P2P_ROOM', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();
      ...
      expect(cm.stateMachine.state).toBe(EState.DIRECT_P2P_ROOM);
      expect(enterRoomSpy).toHaveBeenCalledWith({
        room: 'p2pCallerToCallee',
        participantName: 'User',
        isInPurgatory: false,
        isP2PRoom: false,
        isDirectP2PRoom: true,
      });
    });

    it('эмитит enter-room повторно при смене комнаты в IN_ROOM', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();
      ...
      expect(enterRoomSpy).toHaveBeenLastCalledWith({
        room: 'room-1',
        participantName: 'User1',
        isInPurgatory: false,
        isP2PRoom: false,
      ...
      expect(enterRoomSpy).toHaveBeenCalledTimes(2);
      expect(enterRoomSpy).toHaveBeenLastCalledWith({
        room: 'room-2',
        participantName: 'User2',
        isInPurgatory: false,
    ...
    it('эмитит enter-room с пустыми room и participantName при их отсутствии в контексте машины', () => {
      const { callManager: cm, apiManager } = createManagers();
      const enterRoomSpy = jest.fn();

      cm.on(EEvent.ENTER_ROOM, enterRoomSpy);

      const roomSpy = jest.spyOn(cm.stateMachine, 'room', 'get').mockReturnValue(undefined);
      const participantNameSpy = jest
        .spyOn(cm.stateMachine, 'participantName', 'get')
      ...
      roomSpy.mockRestore();
      participantNameSpy.mockRestore();
    });
  });
});
