...
| `api:participant:move-request-to-spectators`               | Генерируется при запросе перемещения участника в зрители      | `{ isAvailableSendingMedia: boolean; isSynthetic: true } \| { isAvailableSendingMedia: boolean; isSynthetic: false; audioId: string }` |
| `api:participant:move-request-to-spectators-synthetic`     | Генерируется при синтетическом запросе перемещения в зрители  | `{ isAvailableSendingMedia: boolean }`                                                                                                 |
| `api:participant:move-request-to-spectators-with-audio-id` | Генерируется при запросе перемещения в зрители с audioId      | `{ isAvailableSendingMedia: boolean; audioId: string }`                                                                                |
...
### Поля событий перемещения в зрители
